...
for DATASET in "${DATASETS[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        echo "running setting1 residuals on ${DATASET} with seed ${SEED} and backbone ${BACKBONE}"
        python run.py \
            --mode "${MODE}" \
            --id_dataset "${DATASET}" \
            --ood_dataset "${DATASET}" \
...
for DATASET in "${DATASETS[@]}"; do
    for SEED in "${SEEDS[@]}"; do
        echo "running setting2 residuals on ${DATASET} with seed ${SEED} and backbone ${BACKBONE}"
        python run.py \
            --mode "${MODE}" \
            --id_dataset "cifar100" \
            --ood_dataset "${DATASET}" \
